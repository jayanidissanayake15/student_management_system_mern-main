...
import { Users, BookOpen, GraduationCap, FileSpreadsheet, Download, RefreshCw } from 'lucide-react';
...
const LecturerDashboard: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      toast.error('Failed to load lecturer dashboard data');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-8">
      {/* Page Header */}
      <div>
        <h1 className="text-2xl font-bold text-slate-800 tracking-tight font-sans">Lecturer Portal</h1>
        <p className="text-sm text-slate-500">Record daily attendance, publish assignments, enter exam grades, and manage classroom files.</p>
      </div>
      ...
      <div className="glass-card rounded-2xl p-6 border border-slate-100 bg-white flex flex-col sm:flex-row items-center gap-5 shadow-sm">
        ...
        <div className="text-center sm:text-left space-y-1 font-sans">
          ...
          <p className="text-xs text-slate-500 font-semibold">Faculty ID: {profile?.profile?.staffId || 'TBA'}</p>
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
        <StatsCard title="Students Assigned" value={totalStudentsCount} icon={Users} colorClass="text-sky-650" bgClass="bg-sky-50" />
        <StatsCard title="Assigned Subjects" value={subjects.length} icon={BookOpen} colorClass="text-indigo-600" bgClass="bg-indigo-50" />
        <StatsCard title="Averages Passing Rate" value="94%" icon={GraduationCap} colorClass="text-emerald-600" bgClass="bg-emerald-50" />
      </div>

      {/* Subjects Taught & Excel Exports */}
      <div className="glass-card rounded-2xl p-6 border border-slate-100/60 bg-white space-y-4 shadow-sm font-sans">
        <div className="flex justify-between items-center border-b border-slate-100 pb-3">
          <h3 className="font-semibold text-slate-800 text-sm">Active Roster Subjects</h3>
          <button onClick={fetchData} className="text-slate-400 hover:text-indigo-650 transition-all cursor-pointer">
            ...
            subjects.map((sub) => (
              <div key={sub._id} className="border border-slate-100 rounded-2xl p-5 bg-slate-50/50 flex justify-between items-center">
                <div className="space-y-1">
                  <span className="text-[10px] font-bold text-indigo-650 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded-full">
                    {sub.code}
                  </span>
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs bg-white text-slate-700 hover:text-indigo-650 border border-slate-200 hover:border-indigo-200 px-3.5 py-2 rounded-xl transition-all shadow-sm cursor-pointer"
                >
